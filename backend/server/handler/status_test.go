...
func TestStatusHandler_ReportsLiveState(t *testing.T) {
	...
	reg.AddCostUSD(0.25)
	reg.RunStart()
	h := handler.NewStatusHandler(reg, "deepseek-v4-flash")
	...
	for _, want := range []string{`"model_name":"deepseek-v4-flash"`, `"tokens_total":1234`, `"cost_usd":0.25`, `"runs_active":1`, `"connected":true`} {
