#!/usr/bin/env python3
# Vendored from coze-dev/coze-studio (Apache-2.0):
#   backend/infra/coderunner/impl/script/sandbox.py
# Pinned to module github.com/coze-dev/coze-studio/backend v0.0.0-20260420092041-22275b1c2661
# (local clone /home/spud/proj/coze-studio, commit hash matches that pseudo-version).
# It is the Deno + Pyodide WASM sandbox orchestrator spawned by
# infra/coderunner/impl/sandbox.Runner; do not edit unless upgrading the vendored
# copy from coze-studio.
import os
import dataclasses
import json
import subprocess
...
class Sandbox:
    ...
    def _build_command(
        ...
        # Deno >= 2.x enables WASM stack switching by default and rejects the
        # legacy --experimental-wasm-stack-switching flag (removed from V8).
        v8_flags = []
