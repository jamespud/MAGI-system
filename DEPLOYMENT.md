...
| `file_tool` | Built-in `file_query` tool: allow-listed absolute `roots`, traversal rejection, `max_file_bytes` / `max_list_items` bounds. Read/list are always on; `allow_write` / `allow_append` / `allow_delete` / `allow_mkdir` are separate opt-in flags (all default `false`) |
...
| `delegate_tool` | Built-in `delegate` tool (default disabled): spawn independent sub-investigation subagents on the shared agent loop; a `questions` array runs up to 4 in parallel and merges evidence |
...
| `sensor_tool` | External deterministic sensor tool (`run_check`): register linter/compiler/unit-test commands with `name`/`command`/`args`/`timeout_seconds`; only registered commands run, output is fed back for self-correction |
