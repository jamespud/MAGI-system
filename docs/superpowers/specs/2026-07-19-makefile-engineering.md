...
- `clean`: `rm -rf bin frontend/dist`

Note: does NOT run `npm install`. Dependency installation belongs to `prepare`.
Note: does NOT clear Go build/test cache — that stays for fast incremental builds. If needed, a separate `clean-cache` target can be added later.
...
Sources `.env.local` for all configuration values (DB_HOST, DB_PORT, DB_USER, DB_PASS, DB_NAME).
...
Current scope: **backend toolchain only**. Frontend tooling is not yet stable enough
to commit to a specific linter/formatter in `make` targets.
...
When frontend tooling stabilizes, extend the relevant subcommands (e.g. `lint` runs
both golangci-lint and the frontend linter).
...
Exists for future dev-specific overrides (e.g., different env vars, exposed ports).
Initially minimal — an empty services block so the file is valid but has no effect:
...
Generated by `prepare` from `.env.example` if absent. Developer edits for local overrides.
All scripts that need configuration (`dev.sh`, `db.sh`, `docker.sh`) source this file
so there is one canonical place for environment-specific values.
...
- No frontend tooling in `tools.sh` — backend-only until frontend lint/test tooling stabilizes
