...
> 更新记录：2026-08-18 补充 Prompt / Agent Framework / Agent Harness 概念边界对照与六大核心组件级对照，并按 AI Harness 6 大核心组件（Context & Memory / Feedforward & Sensors / Tool & Sandbox / Guardrails & Permissions / Observability & Checkpoint / Evaluation）复核；D1–D17 已修复项状态同步为 ✅/🟡，并新增通用任务执行、反馈传感器、设计模式等缺口行；晚些时候按参考定义补齐 AI 工程范式演进（Prompt→Context→Harness）、确定性矛盾/赛博控速器与 Feedforward-Feedback 控制回路示意，并新增“设计模式”明细行；收尾：新增可编辑调查计划（`/cases/:id/plan`，长任务规划 → ✅）与审计轨迹（`audit_log` + `/admin/audit` + 管理页，组件 4 → ✅），六大组件组件级状态全部升为 ✅；2026-08-19 硬化：NLAH 动作分派已**表驱动**（蓝图声明动作 → 动作注册表分派，未注册动作 fail-fast），并修复三个硬伤（多租户越权、`admin.usage` OOM、任务树增长）；浏览器自动化维持 MCP 集成缝（外部 Playwright/Chrome MCP）。
...
| | 原生文件/代码库/浏览器/DB 工具 | ✅ | `db_tool` 只读 `db_query`（`adapter/db_query_tool.go`）；`file_tool` `file_query` 支持 read/list，写操作（write/append/delete/mkdir）各自独立开关且默认关闭 + symlink 解析防护（`adapter/file_tool.go`）；`repo_tool` 只读 `repo_query`（`adapter/repo_tool.go`）；`web_tool` 受限 `web_fetch`：域名白名单 + 每跳重定向校验 + 解析后 IP 固定（DNS rebinding 防护）+ 大小/超时限制 + HTML 转文本（`adapter/web_fetch_tool.go`）；完整浏览器自动化经 MCP stdio/http 接入外部 Playwright/Chrome MCP server（工具集成缝，非控制面缺口） |
| | Docker / MicroVM 沙箱 | ✅ | `code_runner.docker` Docker 沙箱执行器：`docker run --rm --network none --pids-limit 64 --security-opt=no-new-privileges --read-only --tmpfs /tmp` + 内存/CPU/超时限制 + python/js/bash 解释器 + 可选 `runtime`（如 gVisor `runsc` 轻量虚拟化隔离），复用公共策略校验（`adapter/docker_coderunner.go`）；Firecracker 类独立 MicroVM 仍可后续接入 |
...
| | SSO / OAuth / 自助注册 / 细粒度 RBAC | ✅ | OIDC 授权码流（发现/授权/换 token/userinfo）+ 签名会话 cookie（HMAC）+ 自动开户/自助注册（`application/auth/oidc.go`、`session.go`、`server/handler/oidc.go`）；admin/operator/user 角色 + `RequireAnyRole`（`server/auth.go`、`server/router.go`）；会话 cookie 强制 ≥32 字节 HMAC secret（`application/auth/session.go`）；前端登录页 SSO 入口 |
...
| | **NLAH**（自然语言驱动控制规范） | ✅ | 提示词注册表（D12）+ 角色契约规范（`role_policy`）+ 投票/共识规则规范（`consensus_policy`）+ FSM 编排蓝图：合法转移集合持久化与 admin API，且**每个转移声明 `action` 动作名**（保存时自动回填默认动作、拒绝未知动作，`application/fsmblueprint/`）；orchestrator 经 `ActionFor` 从蓝图解析动作名 → **动作注册表**（`map[action]ActionHandler`，17 个确定性步骤，`domain/orchestration/steps.go`）**表驱动分派**，未注册动作 fail-fast（`domain/orchestration/orchestrator.go`）；同时按蓝图校验状态转移（违规即 fail-fast）。控制面（转移合法性/动作分派/投票/门控/prompt）已由可编辑规范驱动；确定性动作实现保持 Go handler（设计选择，保证确定性） |
| | **自我改进 Harness**（分析失败 → 建议并受控应用规则/prompt） | ✅ | `POST /admin/selfimprove/analyze` 失败分类分析 + 规则/提示改进建议；`POST .../apply` admin 确认写入 prompt registry；`selfimprove.auto_apply_enabled` + 阈值开启全自动规则演化；`selfimprove.mode`（rule/llm/hybrid）控制建议生成：rule 确定性模板、llm 模型即时生成、hybrid 先规则后异步 LLM 增强（`Analyze`/`enhanceAsync`，`application/selfimprove/service.go`） |
