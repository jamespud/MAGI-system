...
| | **NLAH**（自然语言驱动控制规范） | 🟡 | 提示词注册表（D12）+ 角色契约规范（`role_policy`）+ 投票/共识规则规范（`consensus_policy`）+ FSM 编排蓝图：合法状态转移集合持久化、`GET/PUT /admin/fsm-blueprint` 与 reset、`POST /admin/fsm-blueprint/validate` 校验 case 状态历史（`application/fsmblueprint/`、`adapter/fsm_blueprint_repository.go`）；状态机执行逻辑仍为 Go（渐进式迁移） |
