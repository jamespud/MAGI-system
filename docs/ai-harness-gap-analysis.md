...
| **3. Tool & Sandbox** | MCP/API 连接器、文件/代码库/浏览器/DB 工具，Docker/WASM/MicroVM 隔离 | Tavily/Brave 搜索插件、MCP stdio/http、CodeRunner WASM 沙箱、Docker 沙箱、工具策略、只读 DB 与文件查询工具 | 代码库/浏览器工具；MicroVM 沙箱 | 🟡 |
...
| | 原生文件/代码库/浏览器/DB 工具 | 🟡 | `db_tool` 内置只读 `db_query`（`adapter/db_query_tool.go`）；`file_tool` 内置只读 `file_query`：白名单 roots、路径穿越拒绝、大小/条目限制（`adapter/file_tool.go`）；代码库/浏览器工具仍只能靠外部 MCP |
