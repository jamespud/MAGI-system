...
| | **NLAH**（自然语言驱动控制规范） | 🟡 | 提示词注册表（D12）+ 角色契约规范（`role_policy`，`application/rolepolicy/`）+ 投票/共识规则规范：`consensus_policy` 表、`GET/PUT /admin/consensus-policy` 与 reset、orchestrator 运行时覆盖 `ConsensusPolicy`（`application/consensuspolicy/`、`adapter/consensus_policy_repository.go`）；FSM 状态机编排仍为硬编码 Go 规则 |
