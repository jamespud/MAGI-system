...
- Apply `magi_s19_run_admission_lock.sql` and `magi_s20_a2a_output_modes.sql`
  (both additive) before the new binary. S19 is a coordinated behavior cutover:
  drain or gate new submissions until every RunCounter-based replica is gone.
- S16 is forward-only once applied: a binary that does not write `seq` must not
  ...
  tests. If S16 is interrupted mid-way, use
  `docker/atlas/repair/magi_s16_event_sequence_resume.sql` (see
  `docker/atlas/repair/README.md`).
...
**Backend:** Go 1.25 · Hertz (HTTP) · GORM/MySQL · Uber Fx (DI) · eino (LLM) · santhosh-tekuri/jsonschema/v6 · GORM AutoMigrate for non-event tables, with Atlas owning the event sequence migration (`docker/atlas/migrations/magi_s16_event_sequence.sql`).
...
- **OIDC SSO + self-registration** - authorization-code login against any OIDC issuer with signed session cookies that carry only identity + `auth_version`; role/status are re-read from the user store per request, so demote/disable/revoke-sessions (`PATCH /admin/users/:id`, `POST /admin/users/:id/revoke-sessions`) invalidate existing sessions immediately. First login can auto-provision an account, and public self-registration issues a one-time key.
...
- **Two credential kinds** — `auth.static_tokens` are config-declared *service* credentials (role inline, no user row, unaffected by account lifecycle), while DB-issued user API keys are owner-scoped: a DB key only authenticates while its owner exists and is active, and it takes the role from the account, so disabling or deleting a user revokes their keys immediately (store outages fail closed with `503`).
